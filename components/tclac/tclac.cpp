...
void tclacClimate::setup() {
  ...
  // ====== إضافة: تسجيل مستشعر الأمبير الداخلي تلقائياً ======
  if (this->ampere_sensor_ == nullptr) {
    this->ampere_sensor_ = new sensor::Sensor();
    this->ampere_sensor_->set_name("تيار المكيف");
    this->ampere_sensor_->set_unit_of_measurement("A");
    this->ampere_sensor_->set_device_class("current");
    this->ampere_sensor_->set_state_class("measurement");
    this->ampere_sensor_->set_accuracy_decimals(2);
    App.register_sensor(this->ampere_sensor_);
  }
}
...
void tclacClimate::readData() {
  ...
  // ====== إضافة: استخراج قيمة التيار ونشرها ======
  if (this->ampere_sensor_ != nullptr) {
    // افترض أن التيار يأتي في البايت 20 و 21 بصيغة uint16 (ميلي أمبير)
    uint16_t current_mA = (uint16_t(dataRX[20]) << 8) | dataRX[21];
    this->ampere_sensor_->publish_state(current_mA / 1000.0f);
  }
...
void tclacClimate::takeControl() {
  ...
  switch (switch_climate_mode) {
    case climate::CLIMATE_MODE_OFF:
      dataTX[7] += 0b00000000; dataTX[8] += 0b00000000; break;
...
// ====== إضافة: دوال المولد والأمبير ======
void tclacClimate::set_generator_level(int level) {
  generator_level_ = level;
  // إرسال حزمة المولد الخاصة (38 بايت)
  uint8_t msg[38] = {0};
  msg[0] = 0xBB;
  msg[1] = 0x00;
  msg[2] = 0x01;
  ...
  msg[10] = 0x00;
  msg[13] = 0x01;
  msg[18] = level;  // L1=1, L2=2, L3=3

  uint8_t crc = 0;
  for (int i = 0; i < 37; i++) crc ^= msg[i];
  msg[37] = crc;

  sendData(msg, sizeof(msg));
}

void tclacClimate::set_ampere_sensor(sensor::Sensor *sensor) {
  this->ampere_sensor_ = sensor;
}
