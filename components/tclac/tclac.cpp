...
void tclacClimate::set_horizontal_swing_direction(HorizontalSwingDirection hs_direction) {
	this->horizontal_swing_direction_ = hs_direction;
	if (force_mode_status_){
		if (allow_take_control){
			tclacClimate::takeControl();
		}
...
// الدالة الجديدة لاستقبال مستوى وضع المولد من لوحة التحكم لتجنب خطأ الكومبيلر
void tclacClimate::set_gen_mode(int mode) {
	ESP_LOGD("TCL", "تم استقبال أمر وضع المولد بنجاح: %d", mode);
}
